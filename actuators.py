...
# -----------------------------
def lighting(on):
    led_light.value(1 if on else 0) # Lights on

def heating(on):
    led_heat.value(1 if on else 0) # Heat on

def cooling(on):
    led_cool.value(1 if on else 0) # Cool on

def ir_emitter(on=True):
    ir_led.value(1 if on else 0) # IR emitter on (occupancy detecting) 
